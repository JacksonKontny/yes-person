...
def should_approve_pull(
    pull: PullRequest, approval_users: List, comment_limit: int, title_regex=".*"
):
    ...
    user_not_in_approved_comments = get_username_for_token() not in set(
        [comment.username for comment in approved_comments]
    )
    is_title_matching = bool(re.search(re.compile(title_regex), pull.title))
    return (
        user_not_in_approved_comments
        and is_title_matching
        and len(approved_comments) < comment_limit
    )


@cache
def get_username_for_token():
    return requests.get(f"{BASE}/user", headers=get_headers()).json()["login"]


def get_reviews(repo, pr_number, owner="github"):
    return [
        ReviewComment(comment_json)
        for comment_json in requests.get(
